...
1. **Config-driven extraction**: Extraction schemas stored in `CLINICAL_DOCS_EXTRACTION_CONFIG` table (loaded from CSV), not hardcoded. UDFs (`BUILD_DOCUMENT_CLASIFICATION_EXTRACTION_JSON`, `BUILD_DOC_TYPE_EXTRACTION_JSON`) dynamically build `responseFormat` from config at runtime.
...
| Cortex Search Service | 1 | DATA_MODEL_KNOWLEDGE | CLINICAL_DOCS_MODEL_SEARCH_SVC (schema metadata search) |
...
| 7   | DATA_MODEL_KNOWLEDGE.CLINICAL_DOCS_MODEL_REFERENCE refresh | Fully dynamic |
