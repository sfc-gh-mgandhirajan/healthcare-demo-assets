"""
DICOM Ingestion Pipeline - Load DICOM files from S3 directly to Snowflake Repository
Uses boto3 to read files from S3 and parses DICOM metadata with pydicom.
"""
import os
import io
import json
...
S3_BUCKET = 'sf-fcto-mgandhi'
S3_PREFIX = 'dicom_images/'
...
def ingest_dicom_files():
    """Main ingestion function - reads directly from S3."""
    print("=" * 60)
    print("DICOM Ingestion Pipeline (S3 Direct)")
    print("=" * 60)
    
    # Initialize S3 client (uses default AWS credentials)
    print("\nConnecting to S3...")
    ...
    for i, file_info in enumerate(files):
        ...
        if size > 100 * 1024 * 1024:
            print(f"[{i+1}/{len(files)}] Skipping large file: {key} ({size/1024/1024:.1f} MB)")
            continue
        
        print(f"[{i+1}/{len(files)}] Processing: {key} ({size/1024:.1f} KB)")
        ...
        try:
            ...
            metadata = parse_dicom_bytes(dicom_bytes, s3_path)
            if not metadata:
