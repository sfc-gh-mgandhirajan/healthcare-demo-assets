-- Conformance contract tables for the Patient Experience Command Center accelerator.
...
--    The marquee crosswalk is the HCAHPS response scale -> TOP_BOX boolean per measure item.
CREATE TABLE IF NOT EXISTS {{TARGET_DATABASE}}.{{TARGET_SCHEMA}}.ACCELERATOR_CORE_CODE_CROSSWALK (
  ...
  SEMANTIC_ROLE        STRING,                          -- FLAG | CATEGORY | INCLUDE_FILTER | TOPBOX
  NOTES                STRING,
  MAPPING_STATUS       STRING       DEFAULT 'PROPOSED',
  CREATED_AT           TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
  UPDATED_AT           TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
