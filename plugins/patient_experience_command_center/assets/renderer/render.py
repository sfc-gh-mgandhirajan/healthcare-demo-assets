...
def main():
    ...
    for obj in manifest["objects"]:
        ...
        if a.only and name not in a.only:
            continue
        tmpl_name = obj.get("template")
        if not tmpl_name:
            # Externally-produced artifact (e.g., a notebook output / registered model): declared in the
            # manifest for a complete inventory, but there is no SQL to render. Documented, not deployed here.
            skipped.append(name)
            continue
    ...
    print(json.dumps({"rendered": rendered, "skipped_no_template": skipped}, indent=2))
