...
WITH base AS (
    SELECT
        ...
        -- Extract code value from composite (qualifier:value). Handles any sub-element separator.
        NULLIF(REGEXP_SUBSTR(diagnosis_code_1, '[^A-Z0-9]([A-Z0-9.]+)$', 1, 1, 'e', 1), '') AS primary_diagnosis_code,
        NULLIF(REGEXP_SUBSTR(procedure_code, '[^A-Z0-9]([A-Z0-9.]+)$', 1, 1, 'e', 1), '') AS primary_procedure_code,
...
SELECT
    ...
    rendering_provider_last_name,
    rendering_provider_npi,
    NULLIF(REGEXP_SUBSTR(procedure_code, '[^A-Z0-9]([A-Z0-9.]+)$', 1, 1, 'e', 1), '') AS procedure_code,
    TRY_TO_DECIMAL(service_charge_amount, 12, 2) AS service_charge_amount,
