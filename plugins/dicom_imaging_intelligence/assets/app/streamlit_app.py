...
if IS_SIS:
    ...
    @st.cache_resource
    def get_connection():
        # Use SNOWFLAKE_CONNECTION_NAME when set; otherwise fall back to the
        # default connection in ~/.snowflake/connections.toml. Do not hardcode
        # a connection name here - it will not exist in anyone else's account.
...
#
# Queries below write @@FQ@@.OBJECT and @@STAGE@@ instead of hardcoded names,
# and run_query swaps them in at execution time. That keeps the app portable to
# any account without templating this file, and it is done at this single choke
# point rather than by converting every query literal to an f-string - several
# contain braces that an f-string would misparse.
#
# The tokens are deliberately invalid SQL: if a substitution is ever missed, the
# query fails loudly instead of silently resolving against the wrong database.
#
# Resolution order, most to least authoritative:
#   1. app_config.json beside this file - written at deploy time from the
#      plugin's config.json. Deterministic, and the only option that does not
#      depend on session state.
#   2. DICOM_DB / DICOM_SCHEMA env vars - convenient for local development.
#   3. the session's own CURRENT_DATABASE()/CURRENT_SCHEMA().
#
# Order 3 is last on purpose. In Streamlit in Snowflake the app object lives in
# the target schema, so session context is usually right - but "usually" is not
# a good enough basis for something that fails at import if it is wrong.
# -----------------------------------------------------------------------------
_FQ_TOKEN = "@@FQ@@"
_STAGE_TOKEN = "@@STAGE@@"
...
def _app_config():
    """Deploy-time config written next to this file, or {} when absent."""
...
def _resolve_fq():
    cfg = _app_config()
    if cfg.get("database") and cfg.get("schema"):
        return f"{cfg['database']}.{cfg['schema']}"

    env_db = os.getenv("DICOM_DB")
    env_schema = os.getenv("DICOM_SCHEMA")
    if env_db and env_schema:
        return f"{env_db}.{env_schema}"

    ctx = _execute("SELECT CURRENT_DATABASE() AS DB, CURRENT_SCHEMA() AS SCH")
    db = None if ctx.empty else ctx.iloc[0]["DB"]
    schema = None if ctx.empty else ctx.iloc[0]["SCH"]
    if not db or not schema:
        raise RuntimeError(
            "Could not determine which database/schema holds the DICOM objects. "
            "CURRENT_DATABASE()/CURRENT_SCHEMA() are unset for this connection "
            "and no app_config.json was deployed. Set DICOM_DB and DICOM_SCHEMA, "
            "or point your connection at the schema the plugin deployed into."
        )
    return f"{db}.{schema}"
...
@st.cache_resource
def _resolve_stage():
    """The stage to read pixel data from, as '@DB.SCHEMA.STAGE'.

    Read from DICOM_SOURCE_CONFIG rather than hardcoded, so the viewer works
    against a customer's own stage. BUILD_SCOPED_FILE_URL needs a literal stage
    name and cannot take a column value, so this is resolved once here instead
    of being joined per row.

    With more than one enabled source this picks the first by name. A truly
    multi-source viewer would need one query per stage; that is a real
    limitation, not an oversight.
    """
    env_stage = _app_config().get("stage") or os.getenv("DICOM_STAGE")
    if env_stage:
        return env_stage if env_stage.startswith("@") else f"@{env_stage}"

    df = _execute(
        f"SELECT STAGE_FQN FROM {FQ}.DICOM_SOURCE_CONFIG "
        "WHERE ENABLED ORDER BY SOURCE_NAME LIMIT 1"
    )
    if df.empty or not df.iloc[0]["STAGE_FQN"]:
        raise RuntimeError(
            "No enabled row in DICOM_SOURCE_CONFIG, so there is no stage to read "
            "images from. Register a source with the onboard-source skill, or set "
            "DICOM_STAGE to override."
        )
    return f"@{df.iloc[0]['STAGE_FQN']}"


def run_query(sql):
    sql = sql.replace(_FQ_TOKEN, FQ)
    # Resolved lazily: an account that has deployed the pipeline but not yet
    # registered a source can still use every tab that does not read pixels.
    if _STAGE_TOKEN in sql:
        sql = sql.replace(_STAGE_TOKEN, _resolve_stage())
    return _execute(sql)
...
def load_metadata():
    return run_query("""
        ...
        FROM @@FQ@@.DICOM_CARDIAC_METADATA
    """)
...
with st.sidebar:
    ...
    <div style="padding: 0 0.5rem;">
        <div style="font-size: 0.75rem; color: {SF_GRAY_LIGHT} !important; line-height: 1.7;">
            ...
            <span style="color: {SF_BLUE} !important;">&#9679;</span> {FQ}
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.divider()
...
with tab2:
    ...
    with st.expander("Raw DICOM Metadata (JSON)"):
        ...
        sample_row = run_query(f"""
            SELECT RAW_METADATA FROM @@FQ@@.DICOM_CARDIAC_METADATA
...
with tab3:
    ...
    with vc2:
        ...
        def render_slice(_series_label, instance_num, w_center, w_width):
            result = run_query(f"""
                SELECT @@FQ@@.RENDER_DICOM_SLICE(
                    BUILD_SCOPED_FILE_URL(@@STAGE@@, FILE_NAME),
                    {w_center}, {w_width}
                ) AS IMG
                FROM @@FQ@@.DICOM_CARDIAC_METADATA
...
with tab5:
    ...
    proc_df = run_query("SELECT * FROM @@FQ@@.FACT_PROCEDURE")
    ae_df = run_query("SELECT * FROM @@FQ@@.FACT_ADVERSE_EVENT")
    capa_df = run_query("SELECT * FROM @@FQ@@.FACT_CAPA")
    tele_df = run_query("""
        SELECT t.*, d.MODEL_NAME
        FROM @@FQ@@.FACT_DEVICE_TELEMETRY t
        JOIN @@FQ@@.DIM_DEVICE d ON t.DEVICE_ID = d.DEVICE_ID
    """)
    ...
    with ck5:
        with st.container(border=True):
            st.metric("Device Models", run_query("SELECT COUNT(DISTINCT MODEL_NAME) AS C FROM @@FQ@@.DIM_DEVICE").iloc[0]["C"])
    ...
    with st.container(border=True):
        ...
        proc_detail = run_query("""
            ...
            FROM @@FQ@@.FACT_PROCEDURE p
            JOIN @@FQ@@.DIM_DEVICE d ON p.DEVICE_ID = d.DEVICE_ID
            JOIN @@FQ@@.DIM_SITE s ON p.SITE_ID = s.SITE_ID
            LEFT JOIN (
    ...
    if active_query:
        with st.spinner(f"Searching for: *{active_query}*"):
            ...
            search_results = run_query(f"""
                CALL @@FQ@@.SEARCH_DICOM_IMAGES('{safe_q}', 9)
            """)
        ...
        if results_list:
            ...
            for r in results_list:
                series_id = r.get("SERIES_INSTANCE_UID", "")
            ...
            @st.cache_data(ttl=timedelta(hours=1))
            def render_series_middle_slice(series_instance_uid):
                """Render the middle slice of a series for a representative preview."""
                # Series identity comes from the parsed SERIES_INSTANCE_UID tag.
                # This used to be SPLIT_PART(FILE_NAME, '/', 1), which only worked
                # because IDC happens to use the series UUID as its top-level
                # path segment; any other layout broke it.
                safe_uid = str(series_instance_uid).replace("'", "''")
                r = run_query(f"""
                    WITH ordered AS (
                        SELECT FILE_NAME,
                               ...
                               COUNT(*) OVER () AS TOTAL
                        FROM @@FQ@@.DICOM_CARDIAC_METADATA
                        WHERE SERIES_INSTANCE_UID = '{safe_uid}'
                    )
                    SELECT @@FQ@@.RENDER_DICOM_SLICE(
                        BUILD_SCOPED_FILE_URL(@@STAGE@@, FILE_NAME),
                        40, 400
            ...
            for idx, result in enumerate(unique_results):
                ...
                with col:
                    with st.container(border=True):
                        ...
                        img_b64 = render_series_middle_slice(series_instance_uid)
            ...
            with st.container(border=True):
                ...
                series_uids = [
                    str(r.get("SERIES_INSTANCE_UID", "")).replace("'", "''")
                    for r in unique_results
                    if r.get("SERIES_INSTANCE_UID")
                ]
                uid_list = "', '".join(series_uids)
                # Joins straight through SERIES_INSTANCE_UID now. The previous
                # version matched on SERIES_LABEL via a SPLIT_PART subquery,
                # which was both path-dependent and fragile if two series ever
                # shared a label.
                clinical_df = run_query(f"""
                    ...
                    FROM @@FQ@@.DICOM_CARDIAC_METADATA m
                    JOIN @@FQ@@.FACT_PROCEDURE p ON m.PATIENT_ID = p.PATIENT_ID
                    JOIN @@FQ@@.DIM_DEVICE d ON p.DEVICE_ID = d.DEVICE_ID
                    LEFT JOIN @@FQ@@.FACT_ADVERSE_EVENT ae ON p.PATIENT_ID = ae.PATIENT_ID
                    WHERE m.SERIES_INSTANCE_UID IN ('{uid_list}')
                    ORDER BY p.PROCEDURE_DATE
                """)
...
with tab6:
    ...
    IDC Open Data (S3)
        │
        ▼
    External Stage (per DICOM_SOURCE_CONFIG)
        │
        ▼
    Scoped LIST per prefix → DICOM_FILE_MANIFEST
        │
        ▼
...
# The agent lives in the same schema as everything else, so derive its location
# from FQ rather than repeating the database/schema.
AGENT_DB, AGENT_SCHEMA = FQ.split(".", 1)
AGENT_NAME = (
    _app_config().get("agent_name")
    or os.getenv("DICOM_AGENT_NAME")
    or "DICOM_IMAGING_AGENT"
)
