---
name: pipeline-ops
description: Operate the DICOM ingestion pipeline - scan sources, load metadata, replay errors, reconcile deletes, resume or suspend tasks, diagnose a stalled or slow backfill. Use when files are not appearing in the catalog, a load is slow, errors need replaying, or the task graph is suspended. Triggers - pipeline stuck, backfill slow, replay errors, resume tasks, reconcile deletes, files missing from catalog, task suspended.
---

# Operate the ingestion pipeline
