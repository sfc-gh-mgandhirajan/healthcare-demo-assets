---
name: bench
description: Measure and defend cost and performance claims - run ingest benchmarks, compute unit economics, compare billed vs consumed credits, quantify frame plane savings vs a row-per-frame cache. Use when preparing a cost comparison, validating a performance claim, or asked how much this costs. Triggers - benchmark, unit economics, cost per study, cheaper than alternatives, TCO, credits, cost comparison, prove the savings.
---
...
The platform's steady state with zero users is a suspended warehouse and a suspended
compute pool.
...
- the incumbent stores **one row per frame** in a managed Postgres instance and
  ...
  preload.
- The platform stores **zero rows** for any natively-encoded instance, because
...
Be careful with the stronger version of this claim. The platform *does* have an eviction
...
earlier draft of this skill claimed the platform had deleted those operational surfaces
