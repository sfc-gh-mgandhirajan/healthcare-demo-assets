...
A Snowflake-native medical imaging platform. Voxel turns a bucket of DICOM files
...
you need only the header. Voxel's ingest UDTF issues a ranged read, parses up to
...
Voxel stores the two constants per *instance* and derives the rest. Offsets are
...
```bash
ln -s "$PWD" ~/.snowflake/cortex/plugins/voxel-medical-imaging
```

The six skills then resolve as `voxel-medical-imaging:deploy`,
`voxel-medical-imaging:pipeline-ops`, and so on.
