...
│   ├── teardown.sql             # Removes everything the demo creates (idempotent)
...
```sql
-- Run sql/teardown.sql (identifiers are set by configure.py)
```

Removes the agent, the MedGemma SPCS service and compute pool, both MedGemma
external access integrations, the `HIMSS_DEMO` schema (tables, procedures,
semantic view, stage, secret, network rule), and the `MEDGEMMA_DEMO` database.
Every statement uses `IF EXISTS`, so it is safe to re-run.

Two things to know before running it:

- **The SPCS service name is generated by the model-deployment wizard** and includes
  a timestamp (e.g. `MEDGEMMA_4B_IT_V_2026_02_24__17_20_05_SERVICE`), so it cannot be
  templated. Look yours up with `SHOW SERVICES IN COMPUTE POOL MEDGEMMA_COMPUTE_POOL;`
  and substitute it. A compute pool will not drop while a service is attached.
- **It deliberately does not drop `SNOWFLAKE_INTELLIGENCE`, `DEMO_DB`, the warehouses,
  or the account-level `BIND SERVICE ENDPOINT` grant.** Those are shared and the demo
  does not create them — `SNOWFLAKE_INTELLIGENCE.AGENTS` in particular is where every
  agent in the account lives. The script's closing comment block explains each
  exclusion; read it before editing.

Dropping `MEDGEMMA_DEMO` also drops the pushed container image, so rebuilding means
re-deploying the model, which is slow.
